...
/**
 * درگاه mellat (بانکی). تنظیمات: ترمینال/کلید در بخش تنظیمات.
 ...
 */
class BehpardakhtGateway implements PaymentGateway
{
    private Client $http; private string $terminal; private string $key;
    public function __construct(){
        $this->terminal=(string)Setting::get('mellat_terminal','');
        $this->key=(string)Setting::get('mellat_key','');
        $this->http=new Client(['timeout'=>25]);
    }
    public function request(int $amount,string $cb,array $meta=[]):string{
        if(!$this->terminal) throw new \RuntimeException('درگاه mellat پیکربندی نشده است.');
        // ساختار آماده برای اتصال واقعی به API بانک
        throw new \RuntimeException('اتصال درگاه mellat نیازمند پیکربندی ترمینال بانکی است.');
    }
    public function verify(array $r):PaymentResult{
        return new PaymentResult(false,null,'verify درگاه mellat باید مطابق مستندات بانک تکمیل شود.');
    }
}
