...
/**
 * درگاه parsian (بانکی). تنظیمات: ترمینال/کلید در بخش تنظیمات.
...
class ParsianGateway implements PaymentGateway
{
    ...
    public function __construct(){
        $this->terminal=(string)Setting::get('parsian_terminal','');
        $this->key=(string)Setting::get('parsian_key','');
        $this->http=new Client(['timeout'=>25]);
    }
    public function request(int $amount,string $cb,array $meta=[]):string{
        if(!$this->terminal) throw new \RuntimeException('درگاه parsian پیکربندی نشده است.');
        // ساختار آماده برای اتصال واقعی به API بانک
        throw new \RuntimeException('اتصال درگاه parsian نیازمند پیکربندی ترمینال بانکی است.');
    }
    public function verify(array $r):PaymentResult{
        return new PaymentResult(false,null,'verify درگاه parsian باید مطابق مستندات بانک تکمیل شود.');
    }
}
